...
def draw_grid(grid: Grid, path: Path, title: str) -> None:
    ...
    for i in range(10):
        ...
        pos = i * cell
        body.append(f'<line x1="{pos:.1f}" y1="0" x2="{pos:.1f}" y2="{size}" stroke="black" stroke-width="{width}"/>')
        body.append(f'<line x1="0" y1="{pos:.1f}" x2="{size}" y2="{pos:.1f}" stroke="black" stroke-width="{width}"/>')
    for r, row in enumerate(grid):
        for c, value in enumerate(row):
            if value:
                body.append(
                    _svg_text(c * cell + cell / 2, r * cell + cell * 0.67, str(value), 24)
                )
    body.append("</g></svg>")
...
def _bar_chart(
    ...
    solvers = list(data["solver"].drop_duplicates())
    width, height = 900, 480
    left, top, plot_w, plot_h = 80, 55, 750, 320
    ...
    body = [
        ...
        _svg_text(18, top + plot_h / 2, ylabel, 12, "middle").replace("<text", '<text transform="rotate(-90 18 215)"'),
    ]
    for tick in range(5):
    ...
    for i, difficulty in enumerate(difficulties):
        ...
        for j, solver in enumerate(solvers):
            ...
            body.append(
                ...
                f'fill="{COLORS.get(solver, "#444")}"><title>{solver}: {value:.3f}</title></rect>'
            )
    for j, solver in enumerate(solvers):
        y = top + plot_h + 65 + j * 20
        body.append(f'<rect x="{left}" y="{y - 10}" width="12" height="12" fill="{COLORS.get(solver, "#444")}"/>')
        body.append(_svg_text(left + 18, y, solver, 11, "start"))
    body.append("</svg>")
    path.write_text("\n".join(body), encoding="utf-8")
